...
//		String search =fileName[1].substring(fileName[1].indexOf(".")+1);
//		System.out.println(search);
		
		String[]img = {"jpg","png","gif","jpeg"};
		
		System.out.println("====이미지 파일검색===");
		
		for(String tmpFile : fileName) {
			String search =tmpFile.substring(tmpFile.indexOf(".")+1); //search는 확장자만
	...
	//isConiains(String arr[], String search)
	//기능 : 배열에서 찾는 문자열이 있는지 체크 있으면 t 없으면 f
