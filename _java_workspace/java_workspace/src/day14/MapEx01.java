...
public class MapEx01 {
	...
	private int size =5; //별로 좋은 방식은 아님 일단 이렇게.
	...
	public static void main(String[] args) {
		...
		// 1. 저장공간을 어디에 둘지
			//- 1. 멤버변수 => 공용변수로 있는 형태
			//- 2. main (비추천/외부메서드에서 매개변수로 받는 형태)=>매개변수로 받는 형태
			//- 3. 메서드 내부에 생성 -> 리턴의 형태
		// 2. 매개변수와 리턴타입,메서드명을 결정 => interface 생성
		
		MapEx01 ex = new MapEx01(); 
		//map이 static이 아닌 인스턴스변수라서 객체를 반드시 생성해야만 접근 가능
		
		//1번 형태
//		ex.make();
		//멤버변수 출력형태
//		Iterator<String>it=ex.getMap().keySet().iterator(); // 타 클래스인 경우 get으로 접근
...
//		}
	...
	//1. 멤버변수로 map이 있는 형태(공용) 리턴x, 매개변수x
	public void make() {
		while(map.size()<size) {
	...
	//getter setter 필요할 시에(다른 클래스에서 할 경우. map에 직접접근 불가하니까 getMap으로 값 가져오기) 
