...
public class IOStreamEx {
	...
	public static void main(String[] args) {
		 ...
		 * 다양한 장치에 독립적으로 일관성 있는 입출력을 유지하기 위해 입출력스트림을 통해 일관성을 제공
		 * 
		 *  두 가지 형태의 스트림 구분
		 ...
		 *  
		 *  <바이트 스트림>
		 *  - 입력 : InputStream(FileInputStream, BufferInputStream, DataInputStrea)
		 ...
		 *  - 보조스트림 : 직접 읽고 쓰는 기능은 없지만 추가적인 기능을 더해주는 스트림(Buffered붙은 Stream)
		 *  	ex) InputStreamReader, BufferedReader, BufferedWriter ...
		 *  
		 * */
		...
		// 표준 입력(키보드) 스트림
		try {
			int d = System.in.read(); //<- 이 문장이 입력스트림 / 한 바이트 읽기
		} catch (IOException e) {
