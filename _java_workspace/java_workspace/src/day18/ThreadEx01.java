...
public class ThreadEx01 {
	...
	public static void main(String[] args) {
		 ...
		 * - CPU는 시간을 잘게 조개서 thread들을 수행하면 사용자들은 동시에 처리되는 듯한 효과
		 * - thread는 각각 자신만의 작업 공간을 가짐(context)
		 * - 각 thread 사이에 공유하는 자원이 있을 수 있음. (java에서는 static instance)
		 * - 여러 thread가 공유하는 자원 중 경쟁이 발생하는 부분을 critical section(임계영역)
		 * - critical section에 대한 동기화(순차적 수행)를 구현하여 오류를 막음
		 * 
		 * thread 구현 방식 - 1.상속(Thread), 2.interface 구현(Runnable)
		 * 
		 * 
		 * 
		 * */
